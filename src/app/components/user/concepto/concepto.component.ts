...
export class ConceptoComponent implements OnInit {
  ...
  exportPdf(){
    ...
    if(this.rfc){
      this.conceptoService.exportPdf(this.rfc, this.concepto.concepto, this.concepto.nombre, this.empresa.razonSocial, this.registrosTotales).subscribe(
        (data) =>{
          this.blob = new Blob([data], {type: 'application/pdf'});
          var downloadURL = window.URL.createObjectURL(data);
          var link = document.createElement('a');
          link.href = downloadURL;
          link.download = "Reporte de Conceptos al " + fecha.toLocaleDateString('es-MX', options) + ".pdf";
          link.click();
        }
      );
    }
  ...
  exportExcel(){
    ...
    if(this.rfc){
      this.conceptoService.exportExcel(this.rfc, this.concepto.concepto, this.concepto.nombre, this.empresa.razonSocial, this.registrosTotales).subscribe(
        (data) =>{
          this.blob = new Blob([data], {type: 'application/octet-stream'});
          var downloadURL = window.URL.createObjectURL(data);
          var link = document.createElement('a');
          link.href = downloadURL;
          link.download = "Reporte de Conceptos al " + fecha.toLocaleDateString('es-MX', options) + ".xlsx";
          link.click();
        }
      );
    }
